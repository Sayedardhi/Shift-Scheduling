 ...
 * Date: 10th February
 * Course: CS 106B
 * Assignment 6: Recursion to the Rescue
 * Description: This program is an extension to Shift Scheduling to schedule multiple workers rather than a single one
 */
...
/**
     * @brief ifCompatible - A helper function to check if the current shift is compatible with the shifts in `soFar`.
     * @param currentShift: A Shift object representing the current shift.
     * @param soFar: A Set of Shift objects representing the shifts already included in the schedule.
     * @return bool: Returns `true` if the current shift does not overlap with any of the shifts in `soFar`, `false` otherwise.
     */
bool ifCompatible(Shift currentShift, Set<Shift> soFar){
    for (Shift soFarShift : soFar){
        if (overlapsWith(soFarShift, currentShift)){
            return false;
        }
    }
    return true;
...
/**
     * @brief numSchedules - A recursive function to calculate the number of possible schedules with the given shifts and max hours.
     * @param shifts: A constant reference to a Set of Shift objects.
     * @param maxHours: An integer representing the maximum number of hours a schedule can have.
     * @param soFar: A Set of Shift objects representing the shifts already included in the schedule.
     * @return int: Returns the number of possible schedules.
     */
int numSchedules(const Set<Shift>& shifts, int maxHours, Set<Shift> soFar) {
    // base case 1: if there are no more shifts to consider, return 1
    if (shifts.isEmpty()) {
        return  1;
    }

    int schedules = 0;
    Shift firstShift = shifts.first();
    int shiftLength = lengthOf(firstShift);

    //exclude first shift
    schedules += numSchedules(shifts - firstShift, maxHours, soFar);

    // If shift length is within max hours and does not overlap with existing shifts, include it
    if (shiftLength <= maxHours && ifCompatible(firstShift, soFar)){
        schedules += numSchedules(shifts - firstShift, maxHours - shiftLength, soFar + firstShift);
    }
...
/**
 * @brief Calculates the number of schedules for all workers.
 * @param shifts: The set of shifts available.
 * @param maxHours :The maximum number of hours each worker can work.
 * @return Vector<int> The number of schedules for each worker.
 */
Vector<int> numSchedulesForAllWorkers(const Set<Shift>& shifts, const Vector<int>& maxHours) {
    Vector<int> schedules;
    for (int i = 0; i < maxHours.size(); i++) {
        Set<Shift> soFar;
        schedules.add(numSchedules(shifts, maxHours[i], soFar));
    }
    return schedules;
}

/**
     * @brief numSchedulesFor - A wrapper function to calculate the number of possible schedules for the given shifts and max hours.
     * @param shifts: A constant reference to a Set of Shift objects.
     * @param maxHours: An integer representing the maximum number of hours a schedule can have.
     * @return int: Returns the number of possible schedules.
     */
int numSchedulesFor(const Set<Shift>& shifts, int maxHours) {
    Set<Shift> soFar;
    if (maxHours < 0) {
        error("Less than Max Hours");
    }

    return numSchedules(shifts, maxHours, soFar);

}

/**
     * @brief profit - A function to calculate the total profit of a schedule.
     * @param schedule: A Set of Shift objects representing the schedule.
     * @return int: Returns the total profit of the schedule.
     */

int profit(Set<Shift>schedule){
    int count = 0;
    for (Shift currentShift : schedule){
        count += profitFor(currentShift);
    }
    return count;
}
...
/**
     * @brief maxProfitScheduleRec - A recursive function to find the schedule with the maximum profit.
     * @param shifts: A constant reference to a Set of Shift objects.
     * @param maxHours: An integer representing the maximum number of hours a schedule can have.
     * @param soFar: A Set of Shift objects representing the shifts already included in the schedule.
     * @return Set<Shift>: Returns the schedule with the maximum profit.
     */
Set<Shift> maxProfitScheduleRec(const Set<Shift>& shifts, int maxHours, Set<Shift> soFar) {

    if (shifts.isEmpty()) {
        return  soFar;
    }
    //get the first shift and its length
    Shift firstShift = shifts.first();
    int shiftLength = lengthOf(firstShift);

    //exclude the first shift and get the max profit schedule without it
    Set<Shift> excludeShift = maxProfitScheduleRec(shifts - firstShift, maxHours, soFar);
    Set<Shift> includeShift;
    if (shiftLength <= maxHours && ifCompatible(firstShift, soFar)){
        includeShift = maxProfitScheduleRec(shifts - firstShift, maxHours - shiftLength, soFar + firstShift);
    }
    //return the schedule with the highest profit
    if (profit(excludeShift) > profit(includeShift)){
        return excludeShift;
    }else{
        return includeShift;
    }
...
/**
     * @brief maxProfitSchedule - A wrapper function to find the schedule with the maximum profit for the given shifts and max hours.
     * @param shifts: A constant reference to a Set of Shift objects.
     * @param maxHours: An integer representing the maximum number of hours a schedule can have.
     * @return Set<Shift>: Returns the schedule with the maximum profit.
     */
...
Set<Shift> maxProfitSchedule(const Set<Shift>& shifts, int maxHours) {
    ...
    Set<Shift> soFar;
    return maxProfitScheduleRec(shifts, maxHours, soFar);
}
...
STUDENT_TEST("numSchedulesFor handles multiple overlapping shifts that result in an error.") {
    Set<Shift> shifts = {
    ...
    /* With bounded time, no schedule is possible. */
    EXPECT_NO_ERROR(numSchedulesFor(shifts, 6));
}
...
STUDENT_TEST("maxProfitSchedule works for shifts that span multiple days") {
    Vector<Shift> shifts = {
        { Day::MONDAY, 8, 20, 30 }
    };
    EXPECT_EQUAL(maxProfitSchedule(asSet(shifts), 10), asSet({}));
}
